...
export const generatePaletteColors = (
  ...
  switch (strategy) {
    case 'monochromatic':
      ...
      for (let i = 1; i < count; i++) {
        const lightness = Math.max(
          20,
          Math.min(90, lchColor.l + (i - count / 2) * 15)
        )
        const chroma = Math.max(
          0,
          Math.min(132, lchColor.c + (i - count / 2) * 10)
        )
    ...
    case 'complementary':
      ...
      const complementColor = new Color('lch', [
        lchColor.l,
        lchColor.c,
        complementHue,
      ])
      ...
      for (let i = 1; i < Math.floor(count / 2); i++) {
        const lightBase = new Color('lch', [
          Math.min(90, lchColor.l + 15 * i),
          lchColor.c,
          lchColor.h,
        ])
        const lightComplement = new Color('lch', [
          Math.min(90, lchColor.l + 15 * i),
          lchColor.c,
          complementHue,
        ])
    ...
    case 'triadic':
      ...
      if (count > 3) {
        for (let i = 0; i < 3; i++) {
          const hue = (lchColor.h + i * 120) % 360
          const lightColor = new Color('lch', [
            Math.min(90, lchColor.l + 15),
            lchColor.c,
            hue,
          ])
    ...
    case 'split-complementary':
      ...
      if (count > 3) {
        ...
        const lightSplit1 = new Color('lch', [
          Math.min(90, lchColor.l + 15),
          lchColor.c,
          split1,
        ])
        const lightSplit2 = new Color('lch', [
          Math.min(90, lchColor.l + 15),
          lchColor.c,
          split2,
        ])
...
export const checkContrast = (
  foreground: PalettoColor,
  background: PalettoColor
): {
  wcag2: number
  isWCAG2AA: boolean
  isWCAG2AAA: boolean
...
export const getColorAs = (
  color: string | Color | PalettoColor,
  format: string = 'hex'
): string => {
  if (typeof color === 'string') {
    const parsedColor = new Color(color)
    return parsedColor.to('sRGB').toString({ format })
  }
  if (color instanceof Color) {
    return color.to('sRGB').toString({ format })
  }

  return color.color.to('sRGB').toString({ format })
}

// Стратегії палітр з описами
export const paletteStrategies: {
  id: PaletteStrategy
  name: string
  description: string
}[] = [
...
export const getColorName = (color: string | Color) => {
  const oColor = color instanceof Color ? color : new Color(Color.parse(color))
  oColor.alpha = 1 // Set alpha to 1 for color name lookup
  const str = oColor.to('sRGB').toString({ format: 'hex' })
  return nearest(str)?.name
}
